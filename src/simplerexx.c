...
__stdargs BOOL CheckRexxMsg( struct Message *rexxmsg );
__stdargs LONG GetRexxVar( struct Message *rexxmsg, UBYTE *name,
			   UBYTE **result );
__stdargs LONG SetRexxVar( struct Message *rexxmsg, UBYTE *name,
			   UBYTE *value, long length );
#endif

#include	<proto/exec.h>

#include <clib/rexxsyslib_protos.h>
#define RexxSysBase RexxContext->arc_RexxSysBase
...
struct	ARexxContext
{
    ...
    struct Library	*arc_RexxSysBase; /* We will hide the library pointer
					 here... */
    long   Outstanding;		      /* The count of outstanding ARexx
					 messages... */
    char   PortName[24];	      /* The port name goes here... */
    char   ErrorName[32];	      /* The name of the
					 <PortName>.LASTERROR */
